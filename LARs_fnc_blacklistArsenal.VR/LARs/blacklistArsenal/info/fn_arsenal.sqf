...
#define GETDLC\
	{\
		...
		if (count _addons > 0) then {\
			_mods = configsourcemodlist (configfile >> "CfgPatches" >> _addons select 0);\
...
switch _mode do {
	...
	case "ListAdd": {
...

		{
			...
			if (ctrltype _ctrlList == 102) then {_ctrlList lnbsort [1,false];} else {lbsort _ctrlList;};
		} foreach _data;
	};
		...
		if (
			...
			_compatibleItems = _item call bis_fnc_compatibleItems;
			{
				...
				if (_scope == 2 && CONDITION(_virtualItemCargo)) then {
					...
					if (_fullVersion) then {
						private _dlcName = _itemCfg call GETDLC;
						if (_dlcName != "") then {
							_ctrlList lbsetpictureright [_lbAdd,(modparams [_dlcName,["logo"]]) param [0,""]];
						};
